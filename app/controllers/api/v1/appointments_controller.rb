class Api::V1::AppointmentsController < ApplicationController
  ...
  def index
    ...
    elsif params[:status] !='' && params[:centerId].present? && params[:search] !=''
      @count=  Appointment.joins(:patient,:tutor,:appointment_details ).where('appointments.status = ? and branch_office_id=? and patients.name  ILIKE ? or "tutors"."identityCard"  ILIKE ?' ,params[:status],params[:centerId],"%#{params[:search]}%","%#{params[:search]}%" ).uniq.count
      @appointments =   Appointment.joins(:patient,:tutor,:appointment_details ).where('appointments.status = ? and branch_office_id=? and patients.name  ILIKE ? or "tutors"."identityCard" ILIKE ?' ,params[:status],params[:centerId],"%#{params[:search]}%","%#{params[:search]}%").paginate(:page => params[:skip], :per_page => params[:maxCount]).order(:id).uniq
